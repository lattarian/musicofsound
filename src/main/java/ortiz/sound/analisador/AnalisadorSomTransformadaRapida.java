...
import ortiz.sound.analisador.transformadas.TransformadaIn;
import ortiz.sound.analisador.transformadas.TransformadaOut;
import ortiz.sound.analisador.transformadas.TransformadaRapidaFourier;
 ...
 */
public class AnalisadorSomTransformadaRapida implements ObservadorLeituraSom,
	 ...
	 * </p>
	 */
	public AnalisadorSomTransformadaRapida() {
	...
	private void obterFrequencias(byte[] somDigital) throws AnalisadorException {
		...
		double frequencias[] = output.getFrequencias();
		double amplitudes[] = output.getAmplitudes();

		for (int idx = 0; idx < frequencias.length; idx++) {
			listaFrequencias.add(new NotaMusicalFrequenciaSom(amplitudes[idx],
					frequencias[idx]));
		}
